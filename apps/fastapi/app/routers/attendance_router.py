...
from app.schemas.attendance_schema import (
    ...
    AttendanceNonWorkingDatesResponse,
    AttendanceNonWorkingDateItem,
)
...
@router.get("/non-working-dates", response_model=AttendanceNonWorkingDatesResponse)
def get_non_working_dates(
    ...
    try:
        ...
        service = AttendanceService(db)
        blocked = service.get_non_working_dates(
        ...
        return AttendanceNonWorkingDatesResponse(
            dates=[
                ...
                for iso, reason in sorted(blocked.items())
            ]
        )
    except HTTPException:
        raise
