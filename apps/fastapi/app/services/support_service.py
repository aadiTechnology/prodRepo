...
def delete_product_update(db: Session, *, update_id: int, user_id: int) -> str | None:
    row = support_repository.get_product_update_by_id(db, update_id)
    if not row:
        raise NotFoundException("Product update", update_id)
...
def upload_release_note(
    ...
    *,
    ...
    if row.file_path:
        old_disk = disk_path_for_release_note(row.file_path)
...
def get_release_note_disk_path(update_id: int, db: Session) -> tuple[str, str]:
    row = support_repository.get_product_update_by_id(db, update_id)
    ...
    return disk_path, row.file_name or f"release-note-{update_id}"


def get_faq_attachment_disk_path(
    db: Session,
    *,
    faq_id: int,
    attachment_id: int,
    ...
    return disk_path, attachment.file_name
